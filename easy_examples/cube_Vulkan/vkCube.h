...
public:
    ...
    void destroy() {
        if(logicalDevice) {
            logicalDevice->freeMemory(memoryUBO);    // Free memory and destroy UBO
            logicalDevice->destroy(uniformBuffer);
        }
    }
    ...
    void init(vk::PhysicalDevice &phyDev, vk::Device &device, vk::BufferUsageFlagBits buffFlags,
        ...
        uniformBuffer = device.createBuffer(vk::BufferCreateInfo({} , bufferSize, buffFlags));

        const vk::MemoryRequirements memReq = device.getBufferMemoryRequirements(uniformBuffer);
        const vk::MemoryAllocateInfo allocInfo(memReq.size, getMemoryTypeIdx(phyDev, memReq.memoryTypeBits, memFlags));
        memoryUBO = device.allocateMemory(allocInfo);

        device.bindBufferMemory(uniformBuffer, memoryUBO, 0);
    }

    void update() const {
        assert(logicalDevice!=nullptr);

        void *pData = logicalDevice->mapMemory(memoryUBO, 0, bufferSize, vk::MemoryMapFlags(0));
        memcpy(pData, bufferPtr, bufferSize);
        logicalDevice->unmapMemory(memoryUBO); // Unmap after data has been copied
    }

    void copyToDevice(vk::DeviceMemory const & deviceMemory, vk::DeviceSize stride = 0 ) {
        assert(logicalDevice!=nullptr);
        uint8_t * deviceData = static_cast<uint8_t *>( logicalDevice->mapMemory( deviceMemory, 0, bufferSize ) );
...
protected:
    ...
    void updateDescriptorsSets(vk::Buffer &buffer, vk::DeviceSize bufferSize);
...
private:
    ...
    bufferSet uboSceneMat { (void *) &uboMat, sizeof(_uboMat) };
