...
void initVGizmo3D()     // Settings to control vGizmo3D
{
    ...
    // passing the screen sizes calibrate drag rotation and auto-set the mouse sensitivity
        track.viewportSize(initialWindowWidth, initialWindowWidth);      // is necessary also to call when resize window/surface: re-calibrate drag rotation & auto-set mouse sensitivity
...
void mouseMovement()
{
    ...
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        if(glfwGetMouseButton(fwWindow, GLFW_MOUSE_BUTTON_LEFT) != leftPress) {   // check if leftButton state is changed
            ...
            track.mouse(vg::evLeftButton, getModifier(fwWindow),                  // send communication to vGizmo3D...
        ...
        if(glfwGetMouseButton(fwWindow, GLFW_MOUSE_BUTTON_RIGHT) != rightPress) { // same thing for rightButton
            ...
            track.mouse(vg::evRightButton, getModifier(fwWindow),
                                           rightPress, x, y);
        }
        ...
        if(glfwGetMouseButton(fwWindow, GLFW_MOUSE_BUTTON_MIDDLE) != middlePress) {   // check if middleButton state is changed
            ...
            track.mouse(vg::evLeftButton, getModifier(fwWindow),  middlePress, x, y); // call Left activation/deactivation with same "middleStatus"
            track.mouse(vg::evRightButton, getModifier(fwWindow), middlePress, x, y); // call Right activation/deactivation with same "middleStatus"
        }
...
void updateMatrices()
{
    ...
    uboMat.lightMatrix = projMatrix * viewMatrix * compensateView * translationMatrix * (static_cast<mat4>(track.getSecondRot())) * lightObj;
